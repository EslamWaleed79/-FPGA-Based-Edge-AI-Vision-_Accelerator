// =============================================================================
// tb_conv_accelerator.sv
// -----------------------------------------------------------------------------
// Self-checking testbench for top_conv_accelerator.
//
// Reads (produced by golden_model.py):
//   config.txt          -> "H W N OUT_W RELU_EN"
//   input_image.txt      -> H*W decimal unsigned pixels, row-major
//   kernel.txt            -> N*N decimal signed coefficients, row-major
//   expected_output.txt  -> (H-N+1)*(W-N+1) decimal signed pixels, row-major
//
// Drives the DUT: loads the kernel, sets image geometry + ReLU enable,
// pulses start, streams the image 1 pixel/cycle, captures out_valid/
// out_pixel, and compares against expected_output.txt in order.
//
// Prints "TEST PASSED" if every output pixel matches, otherwise prints
// "TEST FAILED" together with the index and values of the FIRST mismatch
// (and continues checking / counts total mismatches for visibility).
// =============================================================================
...
module tb_conv_accelerator;
    ...
    task automatic load_kernel();
        ...
        for (r = 0; r < N; r++) begin
            for (c = 0; c < N; c++) begin
                ...
                cfg_kernel_wr_bank <= 2'b00; // Updated to 2 bits
    ...
    // Limit dump depth to level 2 to avoid parsing internal unsupported primitives
    $dumpvars(2, tb_conv_accelerator.dut);
        ...
        cfg_kernel_sel     = 2'b00; // Updated to 2 bits
        cfg_relu_en        = 1'b0;
