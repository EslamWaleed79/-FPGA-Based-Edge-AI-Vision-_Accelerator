...
module top_conv_accelerator #(
    ...
    // SAME-PADDING (causal, top/left zero-padded -- see line_buffer.sv):
    // every accepted input pixel produces exactly one output pixel, so
    // out_total simply equals pix_total. (Previously, with "valid"
    // convolution, out_total was (W-N+1)*(H-N+1); that shrinkage is gone.)
    // ---------------------------------------------------------------
